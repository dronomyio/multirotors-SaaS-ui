...
function chartRows(chart: ImportedChartModel) {
  ...
  return categories.map((category, index) => {
    ...
    // Explicit type argument required: tsconfig.base.json sets
    // strictFunctionTypes: false, so TypeScript picks the non-generic reduce
    // overload where T = number | null and infers `total` as nullable — even
    // though the callback is annotated `sum: number` and the seed is 0.
